...
def main():
    ...
    """ Setup """
    args = get_args()
    assert args.n_classes in (10, 100)  # valid cifar datasets

    # output
    args.out_path = (
        ('cifar10' if args.n_classes == 10 else '')  # dataset
        + ('cifar100' if args.n_classes == 100 else '')
    ...
    global_model = nn.Sequential(
        ...
        resnet.resnet18(pretrained=False, num_classes=args.n_classes)
    ).cuda(args.gpu_start)
