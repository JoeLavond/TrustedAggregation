...
def main():
    ...
    logger = gu.get_log(args.out_path, f='log_train2.log')
    logger.info(args)
    ...
    np.save(
        os.path.join(args.out_path, 'data', f'output_global_acc{suffix}_v2.npy'), output_global_acc
    )

    output_val_ks = np.array(output_val_ks)
    np.save(
        os.path.join(args.out_path, 'data', f'output_val_ks{suffix}_v2.npy'), output_val_ks
    )

    output_user_ks = np.array(output_user_ks)
    np.save(
        os.path.join(args.out_path, 'data', f'output_user_ks{suffix}_v2.npy'), output_user_ks
    )
