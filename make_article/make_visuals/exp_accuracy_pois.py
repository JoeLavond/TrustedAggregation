...
def main():
    ...
    suffix = f'n_malicious{args.n_malicious}--beta{args.beta}'
    ...
    for index in range(len(titles)):
        ...
        plt.plot(range(0, args.d_rounds + 1), temp_global[:args.d_rounds + 1, 2], '-k')
        ...
        if i == 1 and j == args.m_start:  # only compare base under attacka and defense
            for p, lt, col in zip(base_paths, line_types, line_colors):  # iterate baseline defenses
                ...
                plt.plot(range(0, args.d_rounds + 1), temp_global[:args.d_rounds + 1, 2], c=col, linestyle=lt)
    ...
    plt.savefig(
        os.path.join(tag_path, 'visuals', f'pois_accuracy{suffix}.png'), bbox_inches='tight'
    )
    if args.show:
        plt.show()
    else:
